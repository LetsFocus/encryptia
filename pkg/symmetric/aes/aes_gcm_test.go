...
func TestGCMEncrypt(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Test encryption
			encrypted, err := gcm.Encrypt([]byte(tt.plaintext), tt.key)
			if tt.expectError {
...
func TestGCMDecrypt(t *testing.T) {
	...
	cipharText, _ := gcm.Encrypt([]byte("Hello, World!"), "thisis32bitlongpassphraseimusing")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			decrypted, err := gcm.Decrypt(tt.ciphertext, tt.key)
			if tt.expectError {
				assert.Error(t, err, "Expected an error but got none")
